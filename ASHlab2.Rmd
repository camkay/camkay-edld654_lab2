...
lasso4_rec <- 
  recipe(
    #predicting test scores w/ gender, ethnicity, grade level, and student eligibility for free/reduced lunch
    formula = score ~ gndr + ethnic_cd + enrl_grd + econ_dsvntg, 
    data = data_train #use training set here
  ) %>%
  step_naomit(everything(), skip = TRUE) %>% #omits missing data in predictors
  step_string2factor(gndr, ethnic_cd, econ_dsvntg) %>%  #converts character strings to factors
...
mod_lasso <- linear_reg() %>%
  ...
  set_args(penalty = tune(), # hyperparameter to be tuned
           mixture = 1) # this is what makes it a lasso model
...
```
  ...
  + If this is different than the number of models of `lasso4_fit_1`, please explain why.
    + Yes, the number of models ran was different between models. This is because `lasso4_fit_1` specified a data frame with three possible tuning combinations as an argument to `grid` within the `tune_grid()` function. On the other hand, `lasso4_fit_2` used the default setting, which is ten
...
lasso7_rec <- 
  recipe(
    # predicting test scores w/ gender, ethnicity, grade level, student eligibility for free/reduced lunch,
    # as well as school latitude, school longitude, and whether the student received special ed services & was     # enrolled in general ed less than 40% of the time
    formula = score ~ gndr + ethnic_cd + enrl_grd + econ_dsvntg + lat + lon + dist_sped, 
    data = data_train #training data-set
  ) %>%
  step_naomit(everything(), skip = TRUE) %>% 
  step_string2factor(gndr, ethnic_cd, econ_dsvntg, dist_sped) %>%  # convert character strings to factors
  step_dummy(gndr, ethnic_cd, econ_dsvntg, dist_sped) %>%  #dummy code said variables^
  step_normalize(enrl_grd, lat, lon) #center and standardize numeric variables

# fit new lasso model with 7 predictors
lasso7_fit_2 <- tune_grid(
  object = mod_lasso, # specifying same lasso object where the penalty is set to be tuned.
  preprocessor = lasso7_rec, #using new recipe
  resamples = data_train_r10,
  control = tune::control_resamples(verbose = TRUE,
                                    save_pred = TRUE)
)
...
enet_grid <- grid_regular(enet_params, levels = c(10, 5)) #want 10 different values of penalty and 5 for mixture => 50 models per fold = 500 models total!
