...
lasso4_rec <- 
  recipe(
    # define outcome: test scores
    # define predictors: gender, ethnicity, grade level, and student eligibility for free/reduced lunch
...
mod_lasso <- linear_reg() %>% # using the model for linear regression that allows for penalized regression
  set_engine("glmnet") %>% # setting the engine
  set_mode("regression") %>% # specifying the outcome (regression of classification); redundant w/ above
...
# use a grid search with predefined (default) values to find the optimal tuning parameter values for penalty
...
lasso7_fit_2 <- tune_grid(
  ...
  control = tune::control_resamples(verbose = TRUE, # will print to console model fitting process by fold
                                    save_pred = TRUE)) # will save analysis predictions for each model
...
mod_enet7 <- linear_reg() %>%
  ...
  set_args(penalty = tune(), #placeholder for tuned penalty
           mixture = tune()) #placeholder for tuned mixture; combines L1 (lasso) and L2 (ridge) penalty
